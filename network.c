...
void fuse_conv_batchnorm(network net)
{
    ...
    for(j = 0; j < net.n; ++j){
        ...
        if(l->type == CONVOLUTIONAL){
		    ...
		    l->biases[f] = l->biases[f] - (double)l->scales[f] * l->rolling_mean[f] / (sqrt((double)l->rolling_variance[f] + 0.000001));
		        ...
		        int w_index = f * filter_size + i;
                        ...
                        l->weights[w_index] = (double)l->weights(w_index) * l->scales[f] / (sqrt((double)l->rolling_variance[f] + 0.000001));
